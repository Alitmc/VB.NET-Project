...
Public Class FormRepairElectronicInvoice
    ...
    Private Sub DoPaymentCalculationAction()
        Try
            ...
            If TrnList.Count = 0 Then
                Throw New CustomException("اطلاعاتی جهت اصلاح وجود ندارد.")
            End If
            ...
            If ConfirmMessageBox("آیا از اصلاح صورت حساب اطمینان دارید؟" & vbCr & "تعداد برگه های انتخابی: " & TrnList.Count, True, "") = System.Windows.Forms.DialogResult.Cancel Then
                Return
            End If
            If TrnList.Count > 30 Then
                Throw New CustomException("امکان اصلاح بیش از 30 برگه به صورت یکجا وجود ندارد.")
            End If
            ...
            If TriList.Any(Function(a) a.ProductFinancialStatementCode Is Nothing OrElse (a.ProductFinancialStatementCode.Contains(" "))) Then
                Throw New CustomException("کالا با کد " & ProductList.FirstOrDefault(Function(v) v.ProductID = TriList.FirstOrDefault(Function(a) a.ProductFinancialStatementCode Is Nothing OrElse a.ProductFinancialStatementCode.Contains(" ")).ProductID).ProductCode & " فاقد کد سامانه میباشد و امکان اصلاح وجود ندارد.")
            End If
    ...
    Private Sub btnSend_Click(sender As Object, e As EventArgs) Handles btnSend.Click
        ...
        Try
            ...
            If ConfirmMessageBox("آیا از اصلاح صورت حساب اطمینان دارید؟", True, "") = System.Windows.Forms.DialogResult.Cancel Then
                Return
            End If
            If TrnList.Count > 30 Then
                Throw New CustomException("امکان اصلاح بیش از 30 برگه به صورت یکجا وجود ندارد.")
